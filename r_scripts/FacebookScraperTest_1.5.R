...
metrics = c("page_positive_feedback_by_type")
names(metrics) = c("Page Feedback")
...
for (metric in 1:length(metrics)) {
  ...
  for (week in 0:((nrow(pagedata)/5)-2)) {
    ...
    while(is.null(pull) && attempt <= 100) {
      ...
      if ((seq(from=start, to=(end-1), by = "days")[1] == pull$day[1]) & (seq(from=start, to=(end-1), by = "days")[5] == pull$day[length(pull$day)]) == TRUE) {
  ...
  if ((metrics)[metric] == 'page_positive_feedback_by_type') {
    for (type in 1:length(subtypes)) {
      ...
      colnames(pagedata)[1] = paste0(tolower(names(metrics[metric])),": ",subtypes[type])
      }
  } else {
    pagedata <- cbind(NA,pagedata)
