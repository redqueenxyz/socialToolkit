...
roof = Sys.Date()+1 #"2015-07-31" #Sys.Date()+1
floor = "2014-01-01"
...
for (token in 1:length(ftokens)) {
  ...
  if (me$name == page$from_name[1]) {
    print(paste(me$name,"Dataset exists."))
  } else {
...
metrics = c("page_impressions_unique", "page_impressions_paid_unique", "page_engaged_users", "page_fans","page_consumptions","page_fans_gender_age", "page_positive_feedback_by_type") #c( 
names(metrics) = c("Total Reach",  "Paid Reach", "Engaged Users", "Fans", "Page Consumptions", "Fan Demographics", "Page Feedback") 
...
names(demographics)  <- c("Females 65+", "Females 55-64", "Females 45-54", "Females 35-44", "Females 25-34", "Females 18-24",  "Males 65+", "Males 55-64", "Males 45-54", "Males 35-44", "Males 25-34", "Males 18-24",  "Unidentified 65+", "Unidentified 55-64", "Unidentified 45-54", "Unidentified 35-44", "Unidentified 25-34", "Unidentified 18-24")
  ...
  if (length(metrics) == ((length(pagemetrics)  + length(demographics) ))) {
    break
  } else {
  }
...
metrics <- metrics[-c(which(metrics == "page_positive_feedback_by_type"),which(metrics == "page_fans_gender_age"))]
...
for (pmetric in 1:length(postmetrics)) {
  ...
  for (post in 1:nrow(postdata)) { 
    ...
    if ((substr((postmetrics)[pmetric], 1, 10) == ('post_video')) & (postdata$type[post] == 'video') & (substr(postdata$link[post], 12, 18) != 'youtube') & (substr(postdata$link[post], 8, 15) != 'youtu.be')) {
      } else if ((substr((postmetrics)[pmetric], 1, 10) == ('post_video')) & (postdata$type[post] != 'video')) {
...
#write.xlsx(postdata, paste0(paste(page$from_name[1],"Page Data",Sys.Date(), sep=" "),".xlsx"), row.names=FALSE)
...
#write.xlsx(postdata, paste0(paste(page$from_name[1],"Post Data",Sys.Date(), sep=" "),".xlsx"), row.names=FALSE)

# Raw Data
# output dataset as raw csv
#write.csv(page, paste0(paste(me$name,"Raw Data",Sys.Date(), sep=" "),".csv"),row.names=FALSE,  fileEncoding="utf8")
